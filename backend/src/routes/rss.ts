...
function isDownloadRequested(value: unknown): boolean {
  return (
    typeof value === "string" &&
    ["1", "true", "yes", "on", "ture"].includes(value.toLowerCase())
  );
}
...
async function sendCachedRss(
  ...
  const ownerValue =
    typeof rawQuery.owner === "string" ? rawQuery.owner : "unknown-owner";
  const repoValue =
    typeof rawQuery.repo === "string" ? rawQuery.repo : "unknown-repo";
...
rssRouter.get("/commits", async (req, res, next) => {
  await sendCachedRss(
    ...
    async () => {
      ...
      const targetBranches: Array<string | undefined> = filters.branches ?? [
        undefined,
      ];
      ...
      const dedupedById = new Map<
        string,
        (typeof allCommits)[number][number]
      >();
      ...
      const items: RssItem[] = Array.from(dedupedById.values()).map(
        (commit) => {
...
rssRouter.get("/all", async (req, res, next) => {
  await sendCachedRss(
    ...
    async () => {
      ...
      if (includeCommits) {
        tasks.push(
          fetchCommitsForBranch(query.owner, query.repo, {}).then((commits) =>
            commits.map(normalizeCommitItem),
          ),
        );
      }
      ...
      if (includeIssues) {
        ...
        tasks.push(
          fetchIssues(query.owner, query.repo, issuesState).then((issues) =>
            issues.map(normalizeIssueItem),
          ),
        );
      }
      ...
      if (includePulls) {
        ...
        tasks.push(
          fetchPullRequests(query.owner, query.repo, pullsState).then((pulls) =>
            pulls.map(normalizePullItem),
          ),
        );
      }
      ...
      if (includeReleases) {
        tasks.push(
          fetchReleases(query.owner, query.repo).then((releases) =>
            releases.map(normalizeReleaseItem),
          ),
        );
      }
